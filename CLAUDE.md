...
Fase 0 cerrada (tag `v0.2.1-fase0-enmendado`); Fase 1 cerrada (tag `v0.4.0-fase1`); Fase 2 —adquisición— cerrada (tag `v0.5.0-fase2`); Fase 3 —normalización/validación— cerrada. Fase 4 —protocolo de evaluación— arranca a continuación. Ver `doc/adr/README.md` para el estado exacto de cada decisión. A la fecha de este archivo: nueve de diez ADR cerrados; ADR-008 sigue parcial, con BCR/CEPAL en gestión y la decisión de política de L0 como únicos tramos abiertos — el estado por fuente vive en ADR-008, no acá.

**Antes de escribir cualquier código de estimación o transformación, lee los ADR relevantes en `doc/adr/`.** Son la especificación. No hay decisión metodológica de las diez que debas tomar tú — ya están tomadas y documentadas.
...
R vía `renv`. `renv.lock` está fijado (187 paquetes) y verificado en CI sobre `ubuntu-latest`; `renv::restore()` reproduce el entorno en una máquina limpia. `scripts/bootstrap_renv.R` documenta cómo se generó el lockfile a partir de `DESCRIPTION`, por si hace falta regenerarlo. Paquetes (25 imports declarados en `DESCRIPTION`): `pointblank`, `duckdb`, `seasonal`, `tempdisagg`, `fable`, `tsibble`, `vars`, `tsDyn`, `BVAR`, `midasr`, `glmnet`, `ranger`, `lightgbm`, `xml2`, `httr2`, `chromote`, `jsonlite`, `digest`, `polite`, `readxl`, `urca`, `here`, `testthat`, `dplyr`, `stringr`. **`DESCRIPTION` es la fuente del conteo, no esta lista** — corregido 2026-09-18: este archivo decía 21 y el `README.md` 24, ambos desfasados respecto de las 25 entradas reales del bloque `Imports:`; si la lista de arriba y `DESCRIPTION` discrepan, gana `DESCRIPTION`.
