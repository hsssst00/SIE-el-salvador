...
#   BENCH.ARP_BIC            Δy_t = c + Σ φ_j Δy_{t-j} + e, p ∈ 0..8 por BIC; selección Y estimación
#                            en la misma muestra común (la de p = 8), para que el modelo evaluado sea
#                            exactamente el seleccionado
...
#' Selección de p por BIC sobre la muestra común de p_max (expuesta para pruebas).
seleccionar_ar_bic <- function(dy, p_max = 8L) {
  ...
  mejor <- ajustes[[which.min(bics)]]
  list(p = mejor$p, c0 = unname(mejor$coef[1]), phi = unname(mejor$coef[-1]), bic = bics, n_eff = n_eff)
}
