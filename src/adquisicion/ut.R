...
registrar_ut_demanda_anual <- function(ruta_archivo_local, anio) {
  ...
  nota_anio <- if (anio <= 2003L) {
    ...
    "primera captura - sin vintage previo con el cual comparar."
  }
...
directorio_ut <- "C:/Users/harold/Downloads/total"
for (anio in 2002:2026) {
  ruta <- file.path(directorio_ut, sprintf("%d.csv", anio))
  if (!file.exists(ruta)) {
    stop("FALLO VISIBLE: no se encontró ", ruta, " - confirmar que los 25 archivos están en el directorio antes de correr el lazo completo.")
  }
  registrar_ut_demanda_anual(ruta, anio)
}
