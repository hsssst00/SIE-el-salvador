# Extractor L0 -> L1 para BCR.EXPORT_FOB.NOM.NSA.M (Exportaciones FOB, Balanza Comercial de
# Mercancías, BCR) -- cuarto predictor de la matriz (senda S6.4, comercio exterior), catalogado
# en catalogos/03_series.csv (publicacion_id = BCR.BALANZA_COMERCIAL).
#
# La publicacion trae tres series de cabecera en la misma hoja (Exportaciones FOB, Importaciones
# CIF, Balanza Comercial/saldo) mas un detalle de sub-partidas (cafe, azucar, algodon, camaron,
# Centroamerica/fuera de Centroamerica, maquila). Por decision de Harold (2026-09-16,
# AskUserQuestion), esta primera pasada admite SOLO Exportaciones (FOB) -- Importaciones y el
# saldo quedan como candidatos futuros, no una omision.
#
...
# src/adquisicion/bcr.R. A diferencia de BCR.IPP/ONEC.IPC, esta serie no tiene celdas vacias al
# inicio (arranca ene-1994 con dato real) -- no necesita el forward-fill extendido de
# extraer_bcr_ipp.R/extraer_onec_ipc.R.
#
# Salida: data/L1_staging/BCR_BALANZA_COMERCIAL_series_largo.csv, formato largo (serie_id,
# periodo, valor, provisional). periodo en formato "YYYY-Mnn".
...
series <- series[series$publicacion_id == "BCR.BALANZA_COMERCIAL", ]

if (nrow(series) == 0) {
  stop("FALLO VISIBLE: no hay filas de BCR.BALANZA_COMERCIAL en catalogos/03_series.csv.")
}
...
write.csv(largo, "data/L1_staging/BCR_BALANZA_COMERCIAL_series_largo.csv", row.names = FALSE, na = "")

cat("OK:", nrow(largo), "observaciones,", n_series_obtenidas, "series ->",
    "data/L1_staging/BCR_BALANZA_COMERCIAL_series_largo.csv\n")
