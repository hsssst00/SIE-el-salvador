# Extractor L0 -> L1 para BCR.ITCER.IDX.NSA.M (Indice de Tipo de Cambio Efectivo Real, serie
# global, BCR) -- quinto predictor de la matriz (senda S6.4, tipo de cambio real), catalogado
# en catalogos/03_series.csv (publicacion_id = BCR.ITCER).
#
# La publicacion trae tres series de cabecera en la misma hoja (global, bilateral con Estados
# Unidos, con Centroamerica). Se admite solo la GLOBAL en esta pasada -- es la serie que resume
# competitividad cambiaria ponderada frente a todos los socios comerciales (ver
# 01_publicaciones/BCR.ITCER.yaml), mismo criterio ya aplicado a BCR.BALANZA_COMERCIAL
# (admitir la serie de cabecera mas directa, dejar los desgloses como candidatos futuros, no una
# omision). Autorizado a proceder sin pregunta explicita a Harold para esta tanda de predictores
# (instruccion de la sesion, 2026-09-16).
#
# Mismo mecanismo que src/transformacion/extraer_bcr_ivae.R, duplicado a proposito (ver nota
# equivalente en src/transformacion/extraer_bcr_remesas.R). Sin celdas vacias al inicio (arranca
# ene-2000 con dato real) -- no necesita el forward-fill extendido de extraer_bcr_ipp.R.
#
# Salida: data/L1_staging/BCR_ITCER_series_largo.csv, formato largo (serie_id, periodo, valor,
...
series <- series[series$publicacion_id == "BCR.ITCER", ]
