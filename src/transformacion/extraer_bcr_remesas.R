# Extractor L0 -> L1 para BCR.REMESAS.NOM.NSA.M (Ingresos mensuales de remesas familiares,
# BCR) -- segundo predictor de la matriz (senda S6.4), catalogado en catalogos/03_series.csv
# (publicacion_id = BCR.REMESAS_FAMILIARES_MENSUAL).
#
# Mismo mecanismo que src/transformacion/extraer_bcr_ivae.R (usa los campos estructurados
# hoja/fila_dato/col_inicio/col_fin/fila_anios/fila_trimestres de 03_series.csv para ubicar la
# celda mecanicamente, sin volver a parsear fuente_celda), duplicado en vez de compartido con
# ese extractor a proposito -- mismo criterio ya fijado en src/adquisicion/bcr.R ("una funcion
# por publicacion... para que un cambio de formato en una rompa solo esa").
#
# Salida: data/L1_staging/BCR_REMESAS_series_largo.csv, formato largo (serie_id, periodo, valor,
...
series <- series[series$publicacion_id == "BCR.REMESAS_FAMILIARES_MENSUAL", ]

if (nrow(series) == 0) {
  stop("FALLO VISIBLE: no hay filas de BCR.REMESAS_FAMILIARES_MENSUAL en catalogos/03_series.csv.")
}
...
write.csv(largo, "data/L1_staging/BCR_REMESAS_series_largo.csv", row.names = FALSE, na = "")

cat("OK:", nrow(largo), "observaciones,", n_series_obtenidas, "series ->",
    "data/L1_staging/BCR_REMESAS_series_largo.csv\n")
