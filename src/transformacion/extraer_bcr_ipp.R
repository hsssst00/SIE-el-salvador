# Extractor L0 -> L1 para BCR.IPP.IDX.NSA.M (Indice de Precios al Productor, BCR) -- tercer
# predictor de la matriz (senda S6.4), catalogado en catalogos/03_series.csv (publicacion_id =
# BCR.IPP).
#
# Mismo mecanismo que src/transformacion/extraer_bcr_ivae.R / extraer_bcr_remesas.R (usa los
# campos estructurados hoja/fila_dato/col_inicio/col_fin/fila_anios/fila_trimestres de
# 03_series.csv para ubicar la celda mecanicamente, sin volver a parsear fuente_celda),
# duplicado en vez de compartido a proposito -- mismo criterio ya fijado en
# src/adquisicion/bcr.R ("una funcion por publicacion... para que un cambio de formato en una
# rompa solo esa"). A diferencia de IVAE/REMESAS, col_inicio no es "B": el indice arranca en
# dic-2009=100 por construccion, con once celdas vacias (ene-nov 2009) antes de la primera
# observacion real -- mismo patron ya resuelto por extraer_onec_ipc.R via col_inicio="M", que
# este extractor copia (forward-fill de la fila de anios extendido a toda la fila, no solo al
# rango ci:cf, porque la celda de ano en M no hereda de ningun valor dentro del rango recortado).
#
# Salida: data/L1_staging/BCR_IPP_series_largo.csv, formato largo (serie_id, periodo, valor,
...
extraer_serie <- function(path, hoja, fila_dato, col_inicio, col_fin,
  ...
  # Forward-fill sobre la fila de años COMPLETA (desde la columna 2, la primera con datos; la 1
  # es la etiqueta de fila), no solo sobre el rango ci:cf -- ver nota de cabecera (mismo ajuste
  # que extraer_onec_ipc.R, necesario porque col_inicio (M) no coincide con la primera columna
  # del bloque de su año (B)).
...
series <- series[series$publicacion_id == "BCR.IPP", ]

if (nrow(series) == 0) {
  stop("FALLO VISIBLE: no hay filas de BCR.IPP en catalogos/03_series.csv.")
}
...
write.csv(largo, "data/L1_staging/BCR_IPP_series_largo.csv", row.names = FALSE, na = "")

cat("OK:", nrow(largo), "observaciones,", n_series_obtenidas, "series ->",
    "data/L1_staging/BCR_IPP_series_largo.csv\n")
